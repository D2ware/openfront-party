...
test("GitHub Pages build is subpath-safe and excludes nested repository metadata", () => {
  ...
  assert.equal(chromeManifest.version, "1.0.2");
  assert.equal(chromeManifest.version_name, "1.0.2 Chrome");
  assert.equal(firefoxManifest.version, "1.0.2");
  assert.equal(firefoxManifest.homepage_url, "https://d2ware.github.io/openfront-party/viewer/");
  assert.equal(firefoxManifest.icons[128], "assets/icon-128.png");
  assert.deepEqual(firefoxManifest.host_permissions, ["https://openfront.io/*", "https://relay.example.com/*"]);
  assert.equal(fs.existsSync(path.join(root, "dist", "firefox", "assets", "icon-128.png")), true);
  assert.equal(fs.existsSync(path.join(root, "dist", "openfront-party-firefox-source-1.0.2.zip")), true);
  assert.equal(fs.readFileSync(path.join(output, "extensions", "openfront-party-firefox-source.zip")).subarray(0, 2).toString(), "PK");
  ...
  assert.match(viewerHtml, /id="partyCompanionConsent"/);
  assert.match(viewerHtml, /id="partyCompanionConsentCheck"/);
  assert.match(viewerHtml, /privacy\.html/);
  assert.match(partyClient, /Agree and link OpenFront|startCompanionLink/);
  ...
  assert.doesNotMatch(extensionContent, /\.innerHTML\s*=/);
