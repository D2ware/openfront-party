...
test("GitHub Pages build is subpath-safe and excludes nested repository metadata", () => {
  ...
  assert.equal(chromeManifest.version, "1.0.1");
  assert.equal(chromeManifest.version_name, "1.0.1 Chrome");
  assert.equal(firefoxManifest.version, "1.0.1");
  ...
  assert.equal(fs.existsSync(path.join(root, "dist", "openfront-party-firefox-source-1.0.1.zip")), true);
